...
resident_evil_2_remake_location = {}
resident_evil_2_remake_location.__index = resident_evil_2_remake_location
...
function resident_evil_2_remake_location.new(name)
    local self = setmetatable({}, resident_evil_2_remake_location)
    if name then
...
function resident_evil_2_remake_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = resident_evil_2_remake_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function resident_evil_2_remake_location:connect_two_ways(exit, rule)
...
function resident_evil_2_remake_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function resident_evil_2_remake_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function resident_evil_2_remake_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function resident_evil_2_remake_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function resident_evil_2_remake_location:accessibility()
...
-- 
function resident_evil_2_remake_location:discover(accessibility, keys)
...
entry_point = resident_evil_2_remake_location.new("entry_point")
